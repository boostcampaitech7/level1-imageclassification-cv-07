...
def load_ensemble_config(path):
    ensemble_result_dir = "ensemble_result"
    ...
    ensemble_result_path = os.path.join(py_dir_path, ensemble_result_dir)
    output_dir_path = os.path.join(ensemble_result_path, config["output_dir"])

    output_dir_path = utils.utils.rename_if_exists(output_dir_path)
    output_dir = os.path.split(output_dir_path)[-1]
    output_config_path = os.path.join(output_dir_path, f'{output_dir}.json')
    
    config["output_dir"] = output_dir
    config["output_dir_path"] = output_dir_path
    config["output_path"] = os.path.join(output_dir_path, f'{output_dir}_predict.csv')
    config["output_softmax_path"] = os.path.join(output_dir_path, f'{output_dir}_softmax.csv')
    config["output_config_path"] = output_config_path
...
if __name__ == "__main__":
    ...
    ensemble_config_path = args.config

    config = load_ensemble_config(ensemble_config_path)
    ...
    for result_path in config['result_paths']:
        ...
        predicts[0:predict.shape[0], :] += predict
