...
def inference(model, device, test_loader):
    ...
    with torch.no_grad():
        for images in tqdm(test_loader, desc="Inference"):
            ...
            preds = torch.argmax(outputs, dim=1) #outputs.shape: (batch_size, num_classes)
            predictions.extend(preds.cpu().numpy()) #extend(): list에 list를 추가
    return predictions
...
if __name__ == "__main__":
    ...
    config = {}
    # 실험 설정을 JSON 파일에서 로드하고 환경 변수에서 설정을 가져옴
