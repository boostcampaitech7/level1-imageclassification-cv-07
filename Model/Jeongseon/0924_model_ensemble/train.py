...
def main(exp, config):
    try:
        ...
        if (exp['random_select_dataset']):
            ...
            train_df, val_df = train_test_split(train_info, test_size=exp['valid_split_ratio'], stratify=train_info['target']) # stratify: 클래스 불균형을 고려하여 층화추출
        else:
            train_index = pd.read_csv(exp['train_index_path'], header = None).squeeze() # squeeze(): 1차원 데이터를 Series로 변환
        ...
        # Trainer 설정
        trainer = Trainer(model, device, train_loader, val_loader, optimizer, scheduler, loss_fn, exp['epochs'], exp['result_path']) # Trainer 클래스로 학습 진행
        ...
        for epoch in range(exp['epochs']):
            ...
            log = {
                ...
                "learning_rate": optimizer.param_groups[0]['lr'] #param_groups[0]: 첫 번째 그룹의 모든 파라미터에 대한 정보를 포함 그중에서 lr만
            }
            ...
            # 텍스트 파일에 기록
            if (epoch == 0):
                f = open(exp['log_path'], "w") #파일 쓰기 모드로 열기
                log_writer = csv.DictWriter(f, log.keys()) #log.keys : column 이름으로 사용
                log_writer.writeheader() #첫 줄에 header 쓰기

            log_writer.writerow(log) #에폭마다 한 줄씩 쓰기
            f.flush() #데이터 손실 방지
