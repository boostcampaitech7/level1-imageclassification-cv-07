...
class Slack():
    def __init__(self, token: str, user_id: str):
        self.token = token #슬랙 API 토큰
        self.user_id = user_id #슬랙 사용자 ID
    ...
    def send_dm(self, message: str):
        ...
        if not open_data.get('ok'): #응답이 ok가 아닌 경우
...
def create_path(exp):
    ...
    #실험 설정 딕셔너리에 결과 경로, 폴더명, 예측 결과 파일 경로, 로그 파일 경로 추가
