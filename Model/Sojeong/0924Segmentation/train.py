...
from datasets.segmentation_classification_dataset import SegmentationThenClassificationDataset
...
def main(config):
    try:
        ...
        # 원본 데이터셋 생성
        original_train_dataset = CustomDataset(root_dir=config['train_data_dir'], info_df=train_df, transform=train_transform)
        #original_val_dataset = CustomDataset(root_dir=config['train_data_dir'], info_df=val_df, transform=val_transform)
                
                
        # 원본 이미지 불러오기
        image, _ = original_train_dataset[1]  # 첫 번째 샘플 불러오기 (index 0)        
        # 세그멘테이션 적용
        seg_output = segmentation_model.segment_image(image)
        # 세그멘테이션 시각화
        segmentation_model.save_segmentation(image, seg_output)
        
        
        # segmentation 후 배경 제거된 이미지를 분류 모델로 전달하는 데이터셋 생성
        train_dataset = SegmentationThenClassificationDataset(original_train_dataset, segmentation_model.model, device)
        # Validation 데이터셋은 세그멘테이션을 적용하지 않고 원본 이미지를 그대로 사용
